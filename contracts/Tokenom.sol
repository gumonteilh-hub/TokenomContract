...
contract Tokenom is ERC721Enumerable, Ownable {
    ...
    constructor(string memory name, string memory symbol, uint _attackCooldown, uint32 _maxSpecies) ERC721(name, symbol) {
    ...
    function _mint(address _to, string memory _name) internal {
        ...
        tokenom.name = _name;
        tokenom.level = 1;
        tokenom.maxLifePoint = uint16(_randomNumber(10, 20, Strings.toString(tokenId)));
        tokenom.attacks[0] = 5;
    ...
    function startBattle(uint256 _id, uint256 _versusId) public {
        ...
        require(!tokenomStats[_id].isFighting, "Your tokenom is already fighting");
        require(!tokenomStats[_versusId].isFighting, "Your target is already fighting");
    ...
    */
    function attack(uint256 _id, uint8 _choice) public {
        ...
        if(tokenomStats[_id].cooldown){
            require( (uint256(block.timestamp) - tokenomStats[_id].lastAttack) > attackCooldown , "It's your oponent's turn");
        }

        require(tokenomStats[_id].attacks[_choice] > 0, "This attack does not exist or have not be learned yet by your tokenom");

        if(tokenomStats[tokenomStats[_id].versusId].lifePoint <= tokenomStats[_id].attacks[_choice]) { // victoire
            ...
            tokenomStats[tokenomStats[_id].versusId].lifePoint = tokenomStats[tokenomStats[_id].versusId].maxLifePoint;
            ...
            tokenomStats[tokenomStats[_id].versusId].lifePoint = uint16(tokenomStats[tokenomStats[_id].versusId].lifePoint.sub(tokenomStats[_id].attacks[_choice]));
            ...
            emit AttackSuccess(
                tokenomStats[_id].versusId,
                uint8(tokenomStats[_id].attacks[_choice]),
